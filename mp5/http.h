...
typedef struct link_list_ {
  char *key;
  char *value;
  struct link_list_ *next;
} link_list;

// typedef struct link_list_ link_list;
...
struct _HTTPRequest {
  ...
  // ...however, you MUST keep the `action`, `path`, `version`, and `payload`
  // members
  //    so our test cases can inspect the contents of them without making you
  //    write a bunch of getter functions. :)
};
