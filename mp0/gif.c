 ...
 *
 * Note: To convert a GIF into a global color table, you can use the following
 *`convert` command (via
 *https://legacy.imagemagick.org/discourse-server/viewtopic.php?t=8317): convert
 *tay-og.gif \( -clone 0--1 -background none -append -colors 256 -unique-colors
 *-write mpr:colormap +delete \) -map mpr:colormap tay.gif
 *
 **/
...
static uint8_t decoder[] = {0,  4,  67, 0,   68, 1,  109, 0,  109, 0,  87,
                            2,  60, 1,  116, 0,  32, 0,   63, 1,   61, 1,
                            60, 1,  88, 2,   32, 0,  51,  0,  35,  2};
...
static uint16_t read_num(int fd) {
  ...
  read(fd, bytes, 2); /* Hmm, maybe I should reread the documentation for C's
                          read() call... */
...
gd_GIF *gd_open_gif(const char *fname) {
  ...
  gif->palette = &gif->gct;
  // gif->palette =
  //     0; /* Sometimes, the root cause is many lines above the segfault. */
  gif->bgindex = bgidx;
...
static int read_image_data(gd_GIF *gif, int interlace) {
  ...
  free(table);
  // free(
  //     table); /* If it's good to free once, it must be better to free twice??
  //     */
  if (key == stop)
...
static int read_image(gd_GIF *gif) {
  ...
  /* Assertions will cause your code to break to a debugger, allowing you to
    break when a condition goes outside of an expected range.  Valid file
...
static void del_trie(Node *root, int degree) {
  ...
  free(root);
  return; /* When you get to the end, do it again! */
}
